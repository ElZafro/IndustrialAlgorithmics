...
auto readJobs(const size_t numJobs) {
    ...
    for (size_t i = 0; i < numJobs; ++i) {
        ...
        for (size_t j = 0; j < NUM_MACHINES; ++j) {
            std::cout << "\t\tM" << j << ": ";
...
schedule johnsonAlgorithm(const std::vector<job>& jobs) {

    std::vector<size_t> sequence1{};
    std::vector<size_t> sequence2{};
    for (size_t i = 0; i < jobs.size(); ++i) {
        if (jobs[i].processingTime.front() <= jobs[i].processingTime.back())
            sequence1.push_back(jobs[i].index);
        else
            sequence2.push_back(jobs[i].index);
    }
    std::sort(sequence1.begin(), sequence1.end(), [&](const auto& index1, const auto& index2) { return jobs[index1].processingTime.front() <= jobs[index2].processingTime.front(); });
    std::sort(sequence2.begin(), sequence2.end(), [&](const auto& index1, const auto& index2) { return jobs[index1].processingTime.back() >= jobs[index2].processingTime.back(); });
...
void printSchedule(const schedule& schedule, const std::vector<job>& jobs) {
    ...
    std::vector<size_t> timeMachines(jobs.size(), 0);
...
int main()
{
    std::cout << "This program will construct a schedule of jobs using Johnson's Algorithm.\n";
    std::cout << "Type the number of jobs: ";
    const auto numJobs = readInput<size_t>();
    auto jobs = readJobs(numJobs);

    const auto schedule = johnsonAlgorithm(jobs);
